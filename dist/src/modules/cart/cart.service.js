...
const getCart = async (user) => {
    ...
    const summary = {
        ...
        totalAmount: cartItems.reduce((s, i) => s + Number(i.medicine.price) * i.quantity, 0),
    };
...
const addItemToCart = async (payload, user) => {
...
const updateCartItem = async (medicineId, quantity, user) => {
...
const removeCartItem = async (medicineId, user) => {
    const cartItem = await prisma.cartItem.findUnique({
        where: {
            customerId_medicineId: {
                customerId: user.id,
...
const updateCart = async (user, medicineId, quantity) => {
