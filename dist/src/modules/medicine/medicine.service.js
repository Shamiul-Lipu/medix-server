...
const getMedicineById = async (id, user) => {
...
const createMedicine = async (payload, user) => {
    ...
    const imageUrl = getRandomImage() ?? "https://i.ibb.co.com/0jLd1HNq/product-6.jpg";
...
const updateMedicine = async (id, payload, user) => {
    const medicine = await prisma.medicine.findUnique({
        where: { id },
    });
    if (!medicine) {
...
const deleteMedicine = async (id, user) => {
    ...
    if (activeOrderItems > 0) {
        throw new AppError(400, `Cannot delete medicine with ${activeOrderItems} active orders. Deactivate instead.`);
    }
